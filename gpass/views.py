...
from .serializers import  GpassSerializer
from rest_framework import status

class  GpassViewSet(viewsets.ModelViewSet):
    queryset =  Gpass.objects.all().order_by('-id')  # Order by id in descending order
    serializer_class =  GpassSerializer

    def list(self, request):
        user = request.user
        if user.is_superuser:
            queryset = Gpass.objects.all().order_by('-id')
    ...
    def by_tracknumber(self, request, tracknumber=None):
        letter =  Gpass.objects.get(tracknumber=tracknumber)
    ...
    def sign_document(self, request, pk=None):
        ...
        try:
            document = Gpass.objects.get(id=pk)
            ...
            return Response(serializer.data)
        except Gpass.DoesNotExist:
            return Response({"error": "Gpass with id {} does not exist".format(pk)}, status=status.HTTP_404_NOT_FOUND)
    ...
    def get_documents(self, request):
        user = request.user
        documents = Gpass.objects.none()  
        
        if user.acc_lvl >= 0 and user.acc_lvl <= 2:
            documents = Gpass.objects.all().order_by('-id').filter(to=user.office.officeID)

        user_documents = Gpass.objects.filter(userID=user.id)
